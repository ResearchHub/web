...
type CreateListItemFn = (params: CreateListItemParams) => Promise<ListItem>;
...
export function useCreateListItem(): AsyncState & { createListItem: CreateListItemFn } {
  ...
  const createListItem: CreateListItemFn = useCallback(async (params) => {
...
type DeleteListItemFn = (params: DeleteListItemParams) => Promise<void>;

export function useDeleteListItem(): AsyncState & { deleteListItem: DeleteListItemFn } {
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<AsyncState['error']>(null);

  const deleteListItem: DeleteListItemFn = useCallback(async (params) => {
