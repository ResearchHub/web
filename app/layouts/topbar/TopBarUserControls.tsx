  ...
  <div className="flex items-center">
    <div
      className={cn(
        ...
        variant === 'mobile' ? 'w-10 h-10' : 'w-8 h-8'
      )}
    />
  </div>
);

const DesktopIconSkeleton = () => (
  <div className="h-9 w-9 bg-gray-100 rounded-md animate-pulse flex-shrink-0" />
);
    ...
    <div className="hidden tablet:!flex items-center space-x-2 h-full">
      ...
      {isLoading ? (
        <>
          <DesktopIconSkeleton />
          <DesktopIconSkeleton />
          <AvatarSkeleton variant="desktop" />
        </>
      ) : (
        <>
          {user && (
