...
// Payment Intent Loading Skeleton Component
...
const PaymentForm: FC<{
  ...
  const processingFee = Math.round(inputAmount * PROCESSING_FEE_PERCENTAGE);
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-600 mb-6">
          You need <strong>{rscNeededToBuy.toLocaleString()} RSC</strong> to complete your
          contribution.
        </p>
      </div>
      ...
      >
        {isProcessing ? (
          ...
          `Purchase ${rscNeededToBuy.toLocaleString()} RSC`
        )}
      </Button>
    </form>
  );
...
const PurchaseStep: FC<{
  ...
  // Show skeleton while creating payment intent
  if (isCreatingIntent) {
    return (
      <>
        <ModalHeader
...
// Contribute Step Component
const ContributeStep: FC<{
  onClose: () => void;
  onBuyRSC: () => void;
  onContribute: () => void;
  fundraiseTitle?: string;
...
export const ContributeToFundraiseModalV2: FC<ContributeToFundraiseModalProps> = ({
  ...
  const totalAvailableBalance = Math.round(userBalance + lockedBalance);
  ...
  const processingFee = Math.round(inputAmount * PROCESSING_FEE_PERCENTAGE);
  const totalAmountWithFee = inputAmount + processingFee;

  // Check if user has insufficient balance (including fee)
  const insufficientBalance = totalAmountWithFee > totalAvailableBalance;
  ...
  const handleAmountChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isNaN(numValue)) {
      ...
      if (numValue < 10) {
        ...
        const fee = Math.round(numValue * PROCESSING_FEE_PERCENTAGE);
        const totalWithFee = numValue + fee;

        if (totalWithFee > totalAvailableBalance) {
          setAmountError('Amount exceeds your available balance (including processing fee)');
        } else {
          setAmountError(undefined);
        }
      }
  ...
  return (
    ...
    >
      {currentStep === 'contribute' ? (
        ...
        <PurchaseStep
          onClose={onClose}
          onBack={handleBackToContribute}
          totalAvailableBalance={totalAvailableBalance}
          inputAmount={inputAmount}
          totalAmountWithFee={totalAmountWithFee}
          fundraiseId={fundraise.id?.toString()}
