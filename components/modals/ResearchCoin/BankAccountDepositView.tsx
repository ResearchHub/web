...
interface BankAccountDepositViewProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess?: () => void;
}

export function BankAccountDepositView({
  isOpen,
  onClose,
  onSuccess,
}: BankAccountDepositViewProps) {
  const [isConnecting, setIsConnecting] = useState(false);

  const handleConnectBank = async () => {
    setIsConnecting(true);
    // TODO: Implement Plaid or bank connection flow
  ...
  return (
    <BaseModal
      ...
      footer={
        <Button onClick={handleConnectBank} className="w-full" size="lg" disabled={isConnecting}>
