...
interface DepositRscPanelProps {
  /**
   * Drives the lazy wallet provisioning. The address is only requested once the
   * panel is actually on screen, so mounting it behind a closed modal or an
   * unselected step costs nothing.
   */
  isActive: boolean;
}

/**
 * Deposit address, QR code and fiat on-ramp for topping up an RSC balance.
 *
 * Lives apart from the modal chrome because two surfaces show it: the standalone
 * deposit modal on the wallet page, and the ResearchCoin step inside the add
 * funds flow.
 */
export function DepositRscPanel({ isActive }: DepositRscPanelProps) {
  ...
  // Fetch deposit address when the panel becomes visible (lazy wallet provisioning)
  useEffect(() => {
    if (isActive && !depositAddress) {
