...
const DEFAULT_IMAGE = 'https://images.unsplash.com/photo-1507413245164-6160d8298b31?q=80&w=800' as const;
...
// Reusable Components
const ModalHeader = ({ title, onClose, onBack }: { title: string; onClose: () => void; onBack?: () => void }) => (
...
const PaymentOption = ({ 
  ...
  <button className="flex flex-col items-center justify-center p-6 border border-gray-200 rounded-xl hover:border-gray-300 w-full">
    <div className="w-6 h-6 text-gray-700 mb-3">
      {icon}
    </div>
...
const BalanceInfo = ({ currentBalance, requiredAmount }: { currentBalance: string; requiredAmount: string }) => (
          ...
          <div className="mb-6 flex justify-center">
            {/* add border widht of 2px */}
              <div className="relative w-[240px] h-[240px] overflow-hidden bg-gray-100 rounded-xl border border-gray-200/60 shadow-[0_0_15px_rgba(0,0,0,0.05)] hover:shadow-[0_0_20px_rgba(0,0,0,0.1)] transition-shadow duration-200">
                <Image
                  src={fundingRequest.type === 'funding_request' && 'image' in fundingRequest && fundingRequest.image 
        ...
        <div className="mb-6">
          <Alert variant="special">
            {nftRewardsEnabled ? (
              ...
              <p className="text-green-700">
                Your contribution will directly support groundbreaking research and advance open science.
              </p>
            )}
          </Alert>
        </div>
            ...
            <div className="mt-4 bg-gray-50 rounded-lg p-4 space-y-2">
              <div className="flex justify-between items-center">
                ...
                <span className="text-sm font-medium">
                  {getRscAmount().toLocaleString()} RSC
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-600">NFTs will receive:</span>
                <span className="text-sm font-bold text-blue-600">{getNFTCount().toLocaleString()}</span>
              </div>
            </div>
          )}
        </div>
  ...
  const renderPaymentStep = () => {
    ...
    return (
      <div className="p-6">
        ...
        >
          <div>
            <p>Also earn RSC by contributing to open science through peer reviews, replication studies, and more.</p>
            <Link href="#" className="text-blue-600 hover:text-blue-700">Learn more about earning RSC</Link>
          </div>
        </Alert>
      </div>
    )
