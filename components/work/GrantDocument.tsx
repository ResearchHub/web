...
export const GrantDocument = ({
  ...
  const grantStatus: GrantStatus = work.note?.post?.grant?.status ?? 'CLOSED';
  ...
  const isExpired = grantStatus === 'OPEN' && !isActive;

  const expiringSoon = isExpiringSoon(work.note?.post?.grant?.endDate, 1);
  const display = GRANT_STATUS_CONFIG[grantStatus];
          ...
          <div className="flex-1 text-sm">
            <div className="space-y-2">
              <div className="flex items-center gap-2 text-gray-800">
                <span className={`h-2 w-2 rounded-full ${display.dotClass} inline-block`} />
              ...
              {(grantStatus === 'CLOSED' || isExpired) && endDate && (
