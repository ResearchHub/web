...
import { Coins, FileInput, FileText, Landmark } from 'lucide-react';
import { RegisteredReportStage, RegisteredReportTrackerStep } from '@/types/registeredReport';
...
function getTrackerState(
  ...
  activeStage: RegisteredReportStage
): 'current' | 'complete' | 'missing' {
  if (step.stage === activeStage) return 'current';
  if (step.exists) return 'complete';
  return 'missing';
}

export function RegisteredReportPizzaTracker() {
  const { payload, active, isSwitchingStage, selectTrackerStep } = useRegisteredReportWork();

  return (
    <div className="grid grid-cols-4 gap-0.5 bg-gray-50 py-1">
      {payload.tracker.map((step, stepIndex) => {
        const StepIcon = STAGE_ICONS[step.stage];
        const state = getTrackerState(step, active.stage);
        const isDisabled = !step.exists || !step.url || isSwitchingStage;
        const className = cn(
        ...
        const style = {
          clipPath: resolveTrackerClipPath(stepIndex, payload.tracker.length),
        };
        ...
        return (
          <button
            ...
            type="button"
            disabled={isDisabled}
            onClick={() => selectTrackerStep(step)}
            className={className}
            style={style}
            aria-current={active.stage === step.stage ? 'step' : undefined}
          >
            <StepIcon size={16} />
