...
export type NotebookTab = 'document' | 'ai' | 'details';
...
interface NotebookTabsProps {
  ...
  /** Adds the AI step between Document and Publish. */
  showAi?: boolean;
}

const STEPS: { id: NotebookTab; label: string; dataTour?: string }[] = [
  { id: 'document', label: 'Document' },
  { id: 'ai', label: 'Allocation' },
  { id: 'details', label: 'Publish', dataTour: 'notebook-publish' },
];

/** The steps on offer, in order; the AI step only when asked for. */
export const getNotebookSteps = (showAi = false) =>
  STEPS.filter((step) => step.id !== 'ai' || showAi);

/** Linear step flow shown in the notebook top bar. It reads as a sequence
 *  (Document → Publish) rather than parallel tabs, so the progression toward
 *  publishing is visually explicit. */
export function NotebookTabs({ active, onChange, labels, showAi = false }: NotebookTabsProps) {
  const steps = getNotebookSteps(showAi);
      ...
      {steps.map((step, index) => (
        <div key={step.id} className="flex items-center gap-1.5">
          <Step
            ...
            collapseLabel={steps.length > 2 && step.id !== active}
...
function Step({
  ...
  state: StepState;
  onClick: () => void;
  /** Number only on a phone, where three full labels don't fit the bar. */
