...
export const HubCard: React.FC<HubCardProps> = ({ hub }) => {
  ...
  return (
    <div className="h-full flex flex-col">
      <Link href="/hub/bioscience" className="block h-full">
        <Card className="group hover:shadow-md transition-all duration-200 transform hover:-translate-y-0.5 bg-white border border-gray-200 rounded-lg overflow-hidden flex-1 flex flex-col relative cursor-pointer">
          {/* Journal Badge */}
          {hub.type === 'journal' && (
              ...
              <div className="relative">
                ...
                {hub.type === 'journal' && (
                  <div className="absolute -bottom-1 -right-1">
                    <VerifiedBadge size="sm" isOrganization={true} />
                  </div>
                )}
              </div>
            </div>
