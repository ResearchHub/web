...
export const TopicAndJournalBadge = ({
  ...
  const badgeContent = (
    <>
      {imageUrl && (
        <Avatar src={imageUrl} alt={name} size="xxs" className="ring-1 ring-indigo-200" />
      )}
  ...
  const badgeStyles = `flex items-center gap-1.5 px-2 py-1 rounded-full text-xs font-medium border border-indigo-200 bg-indigo-50 hover:bg-indigo-100 transition-colors ${className}`;
