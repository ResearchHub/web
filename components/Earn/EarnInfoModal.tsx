...
const steps = [
  { icon: Search, text: 'Select a peer-review bounty that matches your expertise' },
  { icon: FileText, text: 'Follow the requirements and submit a peer review' },
  { icon: Coins, text: 'Receive ResearchCoin when your submission is awarded' },
];

interface EarnInfoModalProps {
  isOpen: boolean;
  onClose: () => void;
}

export function EarnInfoModal({ isOpen, onClose }: EarnInfoModalProps) {
  return (
    <BaseModal isOpen={isOpen} onClose={onClose} title="About Peer Reviews" size="md">
      <div>
